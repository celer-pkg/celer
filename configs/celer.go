...
func (c Celer) GenerateToolchainFile(deployMode bool) error {
	...
	toolchain.WriteString(fmt.Sprintf(`# Setup celer during configuration.
if(NOT DEFINED CELER_DEPLOYED)
	...
	if(CELER)
        ...
        set(CELER_DEPLOYED TRUE CACHE INTERNAL "celer already deployed.")
	endif()
endif()`, c.BuildType()) + "\n")
	...
	toolchain.WriteString(fmt.Sprintf(`list(APPEND CMAKE_FIND_ROOT_PATH "%s")`, dependencyDir) + "\n")
	toolchain.WriteString(fmt.Sprintf(`list(APPEND CMAKE_PREFIX_PATH "%s")`, dependencyDir) + "\n")
