...
func (c *Celer) GenerateToolchainFile() error {
	...
	var paths = []string{fmt.Sprintf("${WORKSPACE_DIR}/installed/%s-dev/bin", c.platform.GetHostName())}
	for _, tool := range tools {
		for _, path := range tool.Paths {
			paths = append(paths, fmt.Sprintf("${WORKSPACE_DIR}/downloads/tools/%s", path))
		}
	}
