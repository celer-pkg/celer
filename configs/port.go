...
func (p *Port) findMatchedConfig(buildType string) *buildsystems.BuildConfig {
	for index, config := range p.BuildConfigs {
		if p.checkPatternMatch(config.Pattern) {
			...
			// If LibraryType is empty and not prebuilt, set it to `shared`.
			noLibraryType := strings.TrimSpace(config.LibraryType) == ""
			notPrebuilt := config.BuildSystem != "prebuilt"
			if noLibraryType && notPrebuilt {
