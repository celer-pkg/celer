...
func TestDeployCmd_ArgsValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "dot_export_should_fail",
			setExport:   true,
			exportValue: ".",
			expectError: true,
		},
		{
			name:        "parent_export_should_fail",
			setExport:   true,
			exportValue: "..",
			expectError: true,
		},
		{
			name:        "escape_workspace_should_fail",
			setExport:   true,
			exportValue: "../snapshots/2026-02-21",
			expectError: true,
		},
		{
			name:        "absolute_export_should_fail",
			setExport:   true,
			exportValue: filepath.Join(dirs.WorkspaceDir, "snapshots", "2026-02-21"),
			expectError: true,
		},
		{
			name:        "protected_root_should_fail",
			setExport:   true,
			exportValue: "conf",
			expectError: true,
		},
		{
			...
			expectError:        false,
			expectedExportPath: filepath.Join(dirs.WorkspaceDir, "snapshots", "2026-02-21"),
		},
	}
