...
func (b *BuildConfig) setupEnvs() {
	...
	if b.buildSystem.Name() != "cmake" {
		...
		tmpDevDir := filepath.Join(b.PortConfig.TmpDepsDir, b.PortConfig.HostName+"-dev")
...
func (b BuildConfig) setupPythonEnvs() {
	// Ensure PYTHONUSERBASE for pip.
	...
	if buildtools.PythonTool != nil {
		...
		installedDir := filepath.Join(b.PortConfig.TmpDepsDir, b.PortConfig.LibraryDir)
...
func (b *BuildConfig) appendIncludeDir(includeDir string) {
	...
	case "gcc", "clang", "qcc":
		...
		tmpDepsPrefix := filepath.Join(b.PortConfig.TmpDepsDir, b.PortConfig.LibraryDir)
