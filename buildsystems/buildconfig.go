...
var (
	...
	buildSystems = []string{"cmake", "makefiles", "meson", "qmake", "b2", "gyp", "bazel", "nobuild", "prebuilt", "python", "custom"}
)
...
type PortConfig struct {
	...
	TmpDepsDir      string   // per-port staging root under tmp/deps
...
func (b *BuildConfig) Install(url, ref, archive string) error {
	...
	if toolchain != nil {
		...
		devTmpDepsDir := filepath.Join(b.PortConfig.TmpDepsDir, b.PortConfig.HostName+"-dev")
...
func (b BuildConfig) parseBuildSystem(value string) (name, version string, hasVersion bool, err error) {
	...
	if value == "" {
		return "", "", false, fmt.Errorf("build_system is empty for %s, it should be one of %s",
...
func (b BuildConfig) msvcEnvs() (string, error) {
	...
	tmpDepsDir := filepath.Join(b.PortConfig.TmpDepsDir, b.PortConfig.LibraryDir)
...
func (b BuildConfig) removeLaFiles(root string) error {
