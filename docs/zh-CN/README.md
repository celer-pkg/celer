...
- 🎯 **低侵入**：通过生成 `toolchain_file.cmake` 对接现有工程，不要求你改业务代码或重写构建系统。
- 🚀 **交叉编译**：平台配置中直接描述编译器、sysroot、ABI、环境变量和依赖来源，而不是事后手工拼接。
- 📦 **产物复用**：通过基于哈希的缓存机制复用包产物，减少重复编译和环境漂移。
- 🔧 **多构建工具**：不仅支持 CMake，也支持 Make、Meson、B2、QMake、GYP 等常见构建方式。
- 🏢 **项目隔离**：依赖版本、宏、环境变量和 CMake 变量都跟项目走，避免全局污染和版本互相踩踏。
- 🔗 **团队交付**：生成后的 `toolchain_file.cmake` 可以单独分发，使用方不必强依赖本地安装 Celer。
