...
Celer 的设计目标是通过简化 C/C++ 项目**构建**、**编译**、**安装**的流程复杂性，降低代码库托管、接手和复用的门槛；同时将项目的**依赖管理**与**编译环境**从业务代码中独立出来。这样的职责分离使项目的结构边界更清晰，管理方式也更直观。
...
Celer 根据你的平台和项目配置生成 `toolchain_file.cmake`，把你的工程与预定义的工具链、依赖、环境变量和构建参数连接起来。
...
| **项目隔离** | 共享配置容易导致版本冲突 | 项目维度维护依赖、变量和构建参数，边界更清晰 |
| **多工程协同** | 常需逐项目手工接线 | 单一配置可同步多个子工程，降低维护成本 |
| **私有二进制** | 需要额外封装和流程补丁 | 更适合企业内部制品库、预编译包和定制交付链路 |
| **缓存与重建** | 缺乏团队级制品复用关注 | 基于哈希的制品缓存更强调团队级复用和构建稳定性 |
...
- [构建工具](./article_build_tools.md) - 自动下载和管理构建时工具链
- [导出快照](./cmd_deploy_snapshot.md) - 在部署完成后导出可回溯、可复现的工作区快照
