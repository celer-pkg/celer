...
7. **将当前用户加入 celer 组** — 通过 sudo 运行时使用 `usermod -aG celer $SUDO_USER`，否则回退到 `$USER`
...
4. **卸载已有挂载**: 幂等操作，忽略错误
5. **挂载 NFS 共享**: 在写入 `/etc/fstab` 前先验证服务端导出可访问
6. **校验 `celer` 组 GID**: NFS `sec=sys` 按数字 GID 判断权限，而不是按组名判断。客户端本地 `celer` 组必须和挂载后的导出根目录使用相同的数字 GID。如果本地组不存在且该 GID 未被占用，Celer 会用该 GID 创建；如果已有 `celer` 组使用了不同 GID，setup 会停止并给出修复提示。
7. **创建 celer 系统用户**: 在客户端也创建 celer 系统用户
