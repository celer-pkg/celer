...
- 学习专有的脚本语言或 API
- 手动处理构建流程（configure/build/install）
- 为每个库编写复杂的配方文件

**Celer 的解决方案：**  
只需在简单的 TOML 配置中声明构建系统类型（CMake、Make、Meson 等），Celer 会自动处理其余部分。你只需关注编译选项和依赖关系——无需深入构建系统内部。
...
**问题描述：**  
C/C++ 构建速度臭名昭著，尤其是包含多个依赖的初始构建，传统的预编译解决方案：
- 难以跨多平台维护
- 手动 lib/include 管理容易出错
- 依赖变化时无自动失效机制
- 使用过时或不兼容二进制的风险
...
### 嵌入式系统团队
*“我们支持不限数多个芯片平台。Celer 的平台配置让我们可以立即切换目标，而不会污染彼此的依赖。”*
