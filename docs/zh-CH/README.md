...
- 🎯 **零项目侵入** - 只需一个 `toolchain_file.cmake`，无需修改任何代码
- 🚀 **真正的交叉编译** - 平台感知的依赖管理，预配置工具链
- 📦 **智能缓存** - 基于哈希的二进制制品缓存，节省数小时构建时间
- 🔧 **多构建系统支持** - 原生支持 CMake、Make、Meson、Bazel、B2、QMake、GYP
- 🏢 **企业级就绪** - 项目级配置防止依赖版本冲突
...
| 功能特性 | 你将获得 |
| --- | --- |
| **🔧 可配置化的交叉编译平台** | 通过友好的 TOML 配置预定义 ARM、x86、RISC-V、Windows、Linux 等平台的工具链。 |
| **📁 项目级依赖管理** | 每个项目维护独立的依赖版本、环境变量、宏定义和 CMake 变量 - 避免全局冲突。 |
| **🛠️ 多构建系统支持** | 原生支持 **CMake**、**Makefiles**、**Meson**、**B2**、**Bazel**、**QMake**、**GYP** - 无需编写复杂脚本。 |
...
|-----------|----------------------|---------|
| **添加新库** | 需要复杂的配方脚本 | 只需声明构建系统类型 |
| **多项目平台** | 分散的依赖配置 | 集中式项目 TOML 文件 |
| **版本冲突** | 手动解决冲突 | 自动检测并报告 |
| **构建缓存** | 有限或缺失 | 基于哈希的精确缓存 |
| **跨公司协作** | 手动环境搭建 | 可移植的工具链文件 - 开箱即用 |
| **平台定制** | 全局设置导致冲突 | 项目级隔离 |
...
**高级功能：**
- [生成 CMake 配置文件](./advance_generate_cmake_config.md) - 为非 CMake 库自动生成配置
- [缓存构建产物](./advance_binary_cache.md) - 通过智能缓存加速构建

**深入了解：**
- [为什么选择 Celer？](./why_celer.md) - Celer 独特解决的问题
...
Celer 是一个由社区贡献构建的开源项目。欢迎为以下部分做出贡献：
